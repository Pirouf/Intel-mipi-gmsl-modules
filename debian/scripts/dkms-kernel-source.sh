...
for KVER in "${KVERS[@]}"; do
	...
	echo "dkms-kernel-source.sh: could not parse kernel major version from '${KVER}'" >&2
	exit 1
    fi
    if ! [[ "$minor" =~ ^[0-9]+$ ]]; then
	echo "dkms-kernel-source.sh: could not parse kernel minor version from '${KVER}'" >&2
	exit 1
    fi
